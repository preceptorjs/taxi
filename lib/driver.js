...
var logMethods = require('./log').logMethods;
 ...
 * @module WebDriver
 * @submodule Core
 * @param {String} remote Request URL to selenium-server
 ...
 * @return {Status}
 */
Driver.getStatus = function (remote, options) {
    var request = new Connection(remote, options);

    return when(
        when(request.request('GET', '/status'), Connection.prototype._parseResponse), function (response) {
 ...
 */
Driver.getSessions = function (remote, options) {
    var request = new Connection(remote, options);

    return when(
        when(request.request('GET', '/sessions'), Connection.prototype._parseResponse), function (sessions) {
...
Driver.prototype._getValue = function (value) {
    ...
    switch (this._options.mode) {
        case 'async':
            result = require('promise')(function (resolve) {
                resolve(value);
            });
            break;
        case 'sync':
