...
Screenshot.prototype._determineDevicePixelRatio = function () {
    ...
    return when(this._execute(devicePixelRatioScripts.init), function (initData) {
        ...
        return when(this.takeRawScreenshot(), function (screenshotData) {
            var image = this._screenshotPrePrecessing(PNGImage.loadImageSync(screenshotData));
 ...
 * @private
 */
Screenshot.prototype._needsStitchingExceptions = function (image, initData, devicePixelRatio) {
...
Screenshot.prototype._applyBlockOuts = function (image, blockOuts, devicePixelRatio) {
	blockOuts.forEach(function (blockOut) {
		image.fillRect(
			blockOut.x * devicePixelRatio,
			blockOut.y * devicePixelRatio,
			blockOut.width * devicePixelRatio,
			blockOut.height * devicePixelRatio,
			blockOut.color
		);
	});
};
...
Screenshot.prototype._decodeImages = function (sections) {
    ...
    sections.forEach(function (section) {
        section.viewPorts.forEach(function (viewPort) {
            viewPort.image = this._screenshotPrePrecessing(PNGImage.loadImageSync(viewPort.image));
        }.bind(this));
    }.bind(this));
};
...
Screenshot.prototype._stitchImages = function (area, sections, devicePixelRatio) {
    ...
    sections.forEach(function (section) {
        ...
        section.viewPorts.forEach(function (viewPort) {
            ...
            var offsetX = (section.dstX + viewPort.x) * devicePixelRatio,
                offsetY = (section.dstY + viewPort.y) * devicePixelRatio,
                width = viewPort.width * devicePixelRatio,
                height = viewPort.height * devicePixelRatio;

            viewPort.image.getImage().bitblt(
                image.getImage(),
                viewPort.srcX * devicePixelRatio,
                viewPort.srcY * devicePixelRatio,
                Math.min(width - (section.dstX * devicePixelRatio), viewPort.image.getWidth()),
                Math.min(height - (section.dstY * devicePixelRatio), viewPort.image.getHeight()),
                offsetX,
                offsetY
            );
