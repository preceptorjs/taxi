 ...
 *
 * ###Options
 * * **`blinkDiff.outputOnSuccess`** {`boolean`} - Should an output be created when comparison is successful?
 * There will always be an output if the comparison fails.
 * * **`blinkDiff.failOnDifference`** {`boolean`} - Should Taxi trigger an error when a difference is recognized?
 * * **`blinkDiff.autoApprove`** {`boolean`} - Should Taxi auto-approve screenshots when no previous comparison
 * exist? Otherwise, the test will ignore this comparison, saving the current build screenshot.
 * * **`blinkDiff.approvedPath`** {`string`} - Path to the approved folder. Defaults to the current working directory.
 * * **`blinkDiff.buildPath`** {`string`} - Path to the build folder, holding all the screenshots taken in the recent build.
 * Defaults to the approved folder.
 * * **`blinkDiff.diffPath`** {`string`} - Path to the difference folder, keeping the highlighted differences between
...
Chain.prototype.dialogType = Chain.prototype.dialogSendKeys;
...
Chain.prototype.pause = Chain.prototype.sleep;
